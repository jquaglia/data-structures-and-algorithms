...
const screenForNames = (arr) => {
  // Solution code here...
  const regex = /^(Mr.|Dr.|Mrs.|Ms.) [a-zA-Z]/g;
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const luke = starWarsData[0].mass;
  const names = arr.reduce((acc, value) => {
    if (value.mass > parseInt(luke)) {
      return acc + ' - ' + value.name;
    } else {
      return acc;
    }
  }, '');
  return names.substring(3, names.length);
};
...
const sortBy = (property, arr) => {
  ...
  return arr.sort((first, second) => {
    if (first[property] > second[property]) {
      return 1;
    } else if (second[property] > first[property]) {
      return -1;
    } else {
      return 0;
    }
